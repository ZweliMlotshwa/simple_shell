 ...
 *            by spliting an array string into tokens using a delimiter.
...
char **tokenize(char *str, const char *delim)
{
	...
	while (token)
	{
		...
		ret[i] = malloc(own_strlen(token) + 1);
		if (!(ret[i]))
			return (NULL);

		own_strcpy(ret[i], token);
...
char **tokenize_input(char *input)
{
	...
	tmp = own_strdup(input);
	if (tmp == NULL)
	{
		own_puts("Memory allocation error\n");
		exit(EXIT_FAILURE);
	}
